...
const QuestionsProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const generateStandardQuestions = async (specializationId: string, numQuestionsToGenerate: number): Promise<Question[]> => {
    ...
    try {
        ...
        const newQuestions: Question[] = generatedPayloads.map((payload, index) => ({
          id: `ai-q${timestamp}-${index}-${Math.random().toString(36).substr(2, 9)}`,
          ...payload,
          specialization_id: specializationId,
          is_ai_generated: true,
          choices: payload.choices.map((c, cIndex) => ({ ...c, id: `ai-q${timestamp}-${index}-c${cIndex}-${Math.random().toString(36).substr(2, 5)}` })),
          attachments: [], 
        }));
  ...
  const generatePdfQuestions = async (pdfFile: File, specializationId: string, numQuestionsToGenerate: number): Promise<Question[]> => {
    ...
    try {
        ...
        const timestamp = Date.now();
        const newQuestions: Question[] = generatedPayloads.map((payload, index) => ({
           id: `pdf-ai-q${timestamp}-${index}-${Math.random().toString(36).substr(2, 9)}`,
           ...payload,
           specialization_id: specializationId,
           is_ai_generated: true,
           choices: payload.choices.map((c, cIndex) => ({ ...c, id: `pdf-ai-q${timestamp}-${index}-c${cIndex}-${Math.random().toString(36).substr(2, 5)}` })),
           attachments: [],
        }));
